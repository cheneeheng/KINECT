...
//			if (decideSurface(p, surfaces_eq[ii], surfaces_limit[ii]) &&
			if ((surfaces_mid[ii] - V4d3d((*centroids_)[(int)p[3]])).norm() < 0.15) // HACK: to prevent arbitrary surface detection
			{
		...
		std::shared_ptr<std::vector<Eigen::Vector4d> > centroids_)
{
	...
	if (0)
	{
		...
		std::vector<std::string>goal_action, al;goal_action.resize(10);
		...
		bool flag_)
{
	...
	if (G_->GetNumberOfNodes()==0)
	{
		...
		if(G_->GetObject()==std::string("CUP"))
		{
			...
			goal_action[1] = "SINK";
			goal_action[2] = "FACE";
			goal_action[3] = "TABLE1";
			goal_action[4] = "DISHWASHER";
		}
		else if(G_->GetObject()==std::string("APP"))
		{
			goal_action[0] = "BOWL";
			goal_action[1] = "FACE";
			goal_action[2] = "BIN";
		...
		else
		{
			...
			std::vector<std::string> label_ref {
					al.begin() + kb_->AC()["GEOMETRIC"].first,
					al.begin() + kb_->AC()["GEOMETRIC"].second+1};
			std::cout << label_ref.size() << std::endl;
			std::vector<std::vector<unsigned char> > color_code;
			VTK->ColorCode(color_code);
			VTK->ShowData(
					*points_avg, goal_action, label_ref,
