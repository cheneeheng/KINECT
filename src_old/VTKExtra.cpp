		...
		case CLICK_LABEL:
		{
			...
			for(int i=0;i<num_locations;i++)
			{
					...
					rgb[2]==color_[i+1][2])
				{
					...
					if (!label[i].empty())
					{
						printf(VTKCYEL ">>>>> [WARNING] : Label has been given. Do you wish to overwrite? [Y/N]\n" VTKCNOR);
						printf(">>>>> ");
						while(1)
						{
							...
							{
								printf(VTKCYEL ">>>>> [WARNING] : Label has not been overwritten.\n" VTKCNOR);
								break;
							}
						}
					}
			...
			for(int i = 0;i<num_locations;i++)
			{
					...
					rgb[2]==color_[i+1][2])
				{
					...
					while(1)
					{
						...
						{
							printf(">>>>> [WARNING] : Label has not been deleted.\n");
							printf(">>>>> Press q to quit or click another location with color to delete.\n");
							location[i] = -1;
							break;
						}
...
//	textActor 		= vtkSmartPointer<vtkTextActor>::New();
	...
	actor->SetMapper(this->DataPoints(points_, num_locations, color_, cluster_));
...
//	actor->GetProperty()->SetColor(1.0, 0.0, 0.0);
	...
	else
	{
		for(int i=0;i<num_locations;i++)
		{
			...
			textActor->SetPosition(10, (WIN_HEIGHT-FONT_SIZE-10)-i*FONT_SIZE);
			textActor->GetTextProperty()->SetFontSize(FONT_SIZE);
...
//				}
	...
	if (Show_points)
	{
		...
		vtkSmartPointer<vtkActor> 			actor;
		actor  		= vtkSmartPointer<vtkActor>::New();
		actor->SetMapper(this->DataPoints(points_, num_locations, color_, true));
