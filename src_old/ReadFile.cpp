...
Eigen::Matrix3d rotMat(double angle_, int x_)
{
	Eigen::Matrix3d R_;
	switch (x_)
	{
...
ReadFile::ReadFile() :  list0{}, list1{}, list2{},
						data_line_rf{}, data_word_rf{}
{
		...
		const std::string &path_)
{
	...
	while (src_file)
	{
		...
		while (line_)
		{
		   ...
		   if (data_line_.empty()) data_line_ = word;
		   else data_line_ += (" " + word);
		}
		...
		int &sub_num_)
{
	...
	if (sub_num_==0) {printer(32); return EXIT_FAILURE;	}
	else			 {printer(33);						}
	...
	for(int f=0;f<sub_num_;f++)
	{
		...
		for(int ff=0;ff<nn;ff++)
		{
			...
			for(int fff=0;fff<nnn;fff++)
			{
				...
				map_tmp[c] =
						std::make_pair(
								index[0], dir_s + "/" + list2[fff]->d_name);
				c++;
			}
	...
	std::vector<Eigen::Vector3d> *boxmax_)
{
	...
	else
	{
		...
		Eigen::Vector3d boxmin_tmp, boxmid_tmp, boxmax_tmp, boxmidrot_tmp, y_tmp(0,1,0);
		...
		for(int i=0;i<data_word_rf->size();i++)
		{
			...
			for(int ii=0;ii<3;ii++)
				for(int iii=0;iii<3;iii++)
					srot_[i](ii,iii) = stod((*data_word_rf)[i][14+(ii*3+iii)]);
		}
		...
		std::shared_ptr<CKB> kb_)
{
	...
	else
	{
		...
		for(auto line : *data_word_rf) { sl_.push_back(stod(line[1])); }
		...
		std::shared_ptr<CKB> kb_)
{
	...
	{
		auto s = kb_->AC()["GEOMETRIC"].second-kb_->AC()["GEOMETRIC"].first+1;
			...
			switch (c)
			{
				...
				case 1:
					for(int iii=0;iii<num_location_intervals;iii++)
					{
						...
						{
							edge_tmp.nor[iii][iv] = stod((*data_word_rf)[i][iii*3+iv]);
						}
					}
					break;
				case 2:
					for(int iii=0;iii<num_location_intervals;iii++)
					{
						for(int iv=0;iv<3;iv++)
						{
							edge_tmp.tan[iii][iv] = stod((*data_word_rf)[i][iii*3+iv]);
						}
					}
					break;
				case 3:
					for(int iii=0;iii<num_location_intervals;iii++)
					{
						...
						{
							edge_tmp.loc_mid[iii][iv] = stod((*data_word_rf)[i][iii*4+iv]);
						}
					}
					break;
				case 4:
				...
				case 6:
					...
					{
						edge_tmp.sector_map[iii] = stod((*data_word_rf)[i][iii]);
					}
					break;
			}
		...
		std::shared_ptr<std::vector<std::string> > message_)
{
	...
	{
		for(auto i : *data_line_rf) { message_->push_back(i); }
		...
		std::map<std::string, Eigen::Vector3d> &offset_list_)
{
	...
	for (auto line : *data_word_rf)
	{
		for (int i=0;i<line.size();i++)
		{
			...
			if (word == "Frame") { motion = true; break; }
