...
sem_t lock1, lock2, lock3, lock4, lock5, lock6;
...
{ 
	...
	remove("/home/chen/example.txt");
	remove("/home/chen/recording.txt");
	std::ofstream myfile, write_file;
	myfile.open     ("/home/chen/example.txt"  , std::ios::out | std::ios::app);
	write_file.open ("/home/chen/recording.txt", std::ios::out | std::ios::app);
...
void* contactDetector(void* arg)
{
	while(1)
	{
		...
		contact_obj = contactCheckBox(img_rgb_tmp, "rgb_c", 200, false);
...
void* faceDetector(void* arg)
{
	...
	cv::Mat img_depth_def, mask_obj_def, img_sub, cloud_mask, cloud_mask2, img_tmp;
	cv::Rect face;
...
void* actionRecognition(void* arg)
{
	...
	std::string path;
	std::string PARSED_RES  = "parsed_output.txt";
	...
	path = PARENT + "/" + RESULT + "/" + object + "/ParsedResult/";
	directoryCheck(path);
	...
	while(true)
	{
		...
		// 1. Filter
		T->FilterData(
				Eigen::Vector4d(-gPosition1.x,-gPosition1.y,gPosition1.z,-2),
				(int)contact_obj);
		...
		// 3. Get Data
		T->GetData((int)frame_number_global); counter ++;

		// 4. Parse Data
		T->Parser(PARSED_RES, phrase);
		...
		if (c==30)
		{
			c = 0;
			// 5. Write Results
			T->WriteResult("output.txt", RESULT_DIR, true);
		}
...
void* tts(void* arg)
{
	...
	while(1)
	{
		...
		if (phrase != phrase_last)
		{
			printf("%s\n", phrase.c_str());
			phrase_last = phrase;
			festival_say_text(phrase.c_str());
		}
		sem_post(&mutex_tts);
	}
	return 0;
...
/*
	...
	{
		if (std::string(argv[2]) == "big")
		{
			small_big = true;
			std::cout << "Object is big............" << std::endl;
		}
		else if (std::string(argv[2]) == "small")
		{
			small_big = false;
			std::cout << "Object is small.........." << std::endl;
		}
		else
		{
			std::cerr << "Argument 3 is invalid...." << std::endl;
			std::cerr << "Exiting.................." << std::endl;
			return 0;
		}
	}
	...
	{
		if (std::string(argv[3]) == "objectonly")
		{
			object_only = true;
			std::cout << "Showing object only......" << std::endl;
		}
		else
		{
			std::cerr << "Argument 3 is invalid...." << std::endl;
