...
Test::Test(
		...
		: DF(new DataFilter()),
				APred(
						new ActionPrediction(obj_, loc_int_, sec_int_, KB_, OS_,
								object_state_)),
		...
		const int &num_y_)
{
	...
	if (0)
	{
		...
		VTK->ShowConnectionTest(APred->G.get(), point_zero, label_zero,
				color_code, true);
	}
			...
			if (old_node.name == "FACE")
			{
				...
				for (int ii = 0; ii < APred->G.get()->GetNumberOfNodes(); ii++)
				{
					...
					scale =
							V4d3d(
									face_parser
											- APred->G.get()->GetNode(ii).centroid).norm()
									/ V4d3d(
											old_node.centroid
													- APred->G.get()->GetNode(
															ii).centroid).norm();
				...
				for (int ii = 0; ii < APred->G.get()->GetNumberOfNodes(); ii++)
				{
					...
					R = rodriguezRot(
							V4d3d(
									APred->G.get()->GetNode(ii).centroid
											- old_node.centroid),
							V4d3d(
									APred->G.get()->GetNode(ii).centroid
											- face_parser));
		...
		bool nolabel_)
{
	if (nolabel_)
	{
		WF->WriteFilePrediction(APred->G.get(), APred->KB.get(),
...
//	}
	...
	{
		// 1. Filter
		this->FilterData(points_parser[i], *(APred->pva), contact_parser[i],
				*(APred->contact));
...
//		if(APred->AS->label1>=0)
		{
			...
			tmp.push_back(
					(checkSurfaceDistance((*(APred->pva))[0],
							APred->KB->SurfaceEquation()[0])));
		...
		else if (APred->AS->Probability() < 0)
		{
			labels_predict.push_back(
					APred->KB.get()->AL()[APred->AS->Label2()]);
		}
		else
		{
...
//	plotDatas(title, x, pyy);
	...
	if (0)
	{
		...
		VTK->ShowData(point_zero, goal_action, APred->KB.get()->AL(),
	...
	if (0)
	{
		...
		std::vector<std::vector<unsigned char> > color_code;
		VTK->ColorCode(color_code);
		VTK->ShowConnection(APred->G.get(), point_zero, label_zero, color_code,
				true);
	}
