		...
		std::vector<Eigen::Vector3d> &coeffs_)
{
	...
	if (0)
	{
		...
		for (int i = 0; i < pos_ind_sm.size(); i++)
		{
			P[i][3] = 1;
		}
		...
		VTK->ShowData(P, label_, label_ref, loc_idx_zero, color_code, true,
				false, false);
	}
...
int TrainSM::DecideSectorIntervalExt(
		...
		const int &loc_idx_)
{
	return decideSectorInterval(sec_idx_, delta_t_, point_, edge_->loc_mid,
...
double TrainSM::DecideLocationIntervalExt(
		...
		bool loc_init_)
{
	return dLI(loc_idx_, point_, edge_->loc_len, edge_->loc_mid, edge_->tan,
		...
		int loc_offset_)
{
	...
	if (this->DecideLocationIntervalExt(edge_, point_, loc_idx, loc_last_idx_,
			loc_offset_, loc_init_) > 0.001)
	{
		...
		int loc_offset_)
{
	...
	tmpdlie = this->DecideLocationIntervalExt(edge_, point_, loc_idx,
		...
		std::vector<Eigen::Vector3d> coeffs_)
{
	...
	auto edge_tmp = std::make_shared<CGraph::edge_t>(
			G_->GetEdge(label1_sm, label2_sm, 0));
	auto edge_tmp_mem = std::make_shared<CGraph::edge_t>(
			G_->GetEdge(label1_sm, label2_sm, 0));
...
//	edge_tmp.total_len = total_len;
	...
	for (int i = 0; i < loc_int; i++)
	{
		...
		for (int ii = mem; ii < integral_limit_ * 100 + 1; ii++)
		{
			...
			if (tl >= len[0] && lim[0] < 0.0)
			{
				lim[0] = t;
			}
			else if (tl >= len[1] && lim[1] < 0.0)
			{
				lim[1] = t;
			}
			else if (tl >= len[2] && lim[2] < 0.0)
			{
				lim[2] = t;
				mem = ii;
				break;
		...
		if (N == 0)
		{
			...
			else
			{
				...
				edge_tmp->tan[i] = p_tan.normalized();
				Eigen::Matrix3d R = rodriguezRot(edge_tmp->tan[0],
						edge_tmp->tan[i]);
		...
		edge_tmp->loc_len[i] = ((p_mid + (p_tan * (lim[0] - lim[1])))
	...
	if (N > 0)
	{
		...
		{
			edge_tmp->loc_mid[l] = ((N / (N + 1))
					* (edge_tmp_mem->loc_mid[l] * edge_tmp_mem->loc_mid[l][3]
							+ node_tmp.centroid))
					+ ((1 / (N + 1))
							* (edge_tmp->loc_mid[l] * edge_tmp->loc_mid[l][3]
									+ node_tmp.centroid));
			...
			else
			{
				...
				edge_tmp->nor[l] = R.inverse().transpose()
						* edge_tmp_mem->nor[l];
			}
		}
		// ***********************************************************[AVERAGE]
		...
		for (int l = 0; l < loc_int; l++)
		{
			...
			{
				// [OLD POINT]*************************************************
				aa.angle() = 2 * M_PI * fmod((s + 0.5), (double) sec_int)
						/ sec_int;
				...
				p_old = (edge_tmp_mem->loc_mid[l] * edge_tmp_mem->loc_mid[l][3])
						+ V3d4d(
								tmpN
										* edge_tmp_mem->sector_map[l * sec_int
												+ s]);
				// *************************************************[OLD POINT]
				bool tmp_init = false;
				int last; //##TODO
		...
		bool &loc_init_)
{
	...
	{
		return EXIT_FAILURE;
	} // TO BIGGGGGGGGG change too big possible for new edge
		...
		const int &loc_offset_)
{
	...
	{
		if (idx == 0 || idx == pos_ind_sm.size() - 2)
		{
			continue;
		}
		idx++;
		...
		if (this->FitSectorMap(edge_tmp,
				pos_ind - G_->GetNode(label1_sm).centroid, loc_last_idx, offset,
		...
		if (0)
		{
			...
			VTK->ShowConnection(G_.get(), point_zero, label_zero, color_code,
					true);
		}
	}
...
//		this->FitSectorMapInit(G_, points_avg_, loc_int/2);
	...
	else if (G_->GetEdgeCounter(label1_sm, label2_sm, 0) < 50)
	{
		...
		if (0)
		{
			...
			VTK->ColorCode(color_code);
			std::vector<std::string> label_zero;
			VTK->ShowConnection(G_.get(), pos_ind_sm, label_zero, color_code,
					true);
		}
		this->AdjustCurveExt(G_, coeffs);
		//VISUALIZE
			...
			else
			{
				...
				if (label_idx_sm > 0)
				{
					...
					if (i - label_idx_sm > 10)
					{
						...
						{ };
						pos_ind_sm.insert(pos_ind_sm.begin(),
								G_->GetNode(label1_sm).centroid);
						pos_ind_sm.push_back(G_->GetNode(label2_sm).centroid);
						...
						if (0)
						{
							auto VTK = std::make_shared<VTKExtra>(loc_int,
									sec_int);
							std::vector<std::vector<unsigned char> > color_code;
							VTK->ColorCode(color_code);
							std::vector<std::string> label_zero;
							VTK->ShowConnection(G_.get(), pos_ind_sm,
									label_zero, color_code, true);
						}
